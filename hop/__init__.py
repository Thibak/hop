...
# - 
# Работа с обратными связями. 
# self.Engine.FB.
#  - changeDict(имя словаря)

# Управление сервером обратных связей:
# e.FB.
#  -  addDict(name, dict = {}) -- где dict - словарь, т.е. пары имя_слота:строка_для_выполнения. Имя слота -- имя по которому происходит обращение из событий, строка -- функционал, возвращающий управляющее значение. В общем случае может содержать все, что угодно. В частном предпологается использование сумматоров из компартментов по формату __________________________________
...
# Бывает абстрактный контейнер события (например генерирующий клетку-мутанта) -- EventContainer(Time, st), принимающий на вход время, когда он должен быть выполнен и строку, что собственно он должен сделать.

<--- тут надо подумать, что-то какая-то ерунда. Можно просто хранить время до ограничителя и автоматом добавлять его при ините движка
# либо генератор технических событий RoutineEvents (принимающий теже параметры), пока содержащий всего один полезный метод -- стоп-машина. Без такого события расчет длится бесконечно. Если клетки не элиминируют естественно.

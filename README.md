...
Далее:    
    ...
    переименовать
    проверить систему названий в соответствии с новой номенклатурой   
...
--------------------------------
Обнаружилось, что с некоей ненулевой вероятностью случайное ожидание для первой клетки может сразу превышать время останова (распр. Вейбулла же тяжелохвосто!), в этом случае на выходе мы получаем нулевой длины таймлайн. Ситуация может и экзотическая, но приводит к вылету монтекарловского скрипта, т.к.он не периваривает нулевых массивов (что идиотизм, конечно). 
Внимание вопрос, в каком месте делать заплатку. 
1. прибавлять в начале (в конце?) к таймлайну None-значение, дабы избавится от пустых массивов таймлайна -- не катит, т.к. минимумы и нумпая и дефолтный воспринимают его не как миссинг, что стоило бы, а как минимальное значение.
2. А отчего не генерит мой останов такой же дельтатайм? А стоило бы....
